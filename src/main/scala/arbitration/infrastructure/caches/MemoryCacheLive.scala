...
final class MemoryCacheLive[K, V](cache: Cache[K, MarketError, V])
  ...
  override def getOrFetch(key: K): ZIO[Any, MarketError, V] =
    ...
    cache
      .get(key)
      .tapError(e =>
        ZIO.logErrorCause(s"Failed to get cache for key $key", Cause.fail(e))
      )
      .mapError {
        case _: NoSuchElementException =>
          NotFound(s"Cache for key $key not found")
        case other => other
      }
